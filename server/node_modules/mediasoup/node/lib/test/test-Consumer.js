...
afterEach(async () => {
    ...
    if (ctx.worker?.subprocessClosed === false) {
        await new Promise(resolve => ctx.worker?.on('subprocessclose', resolve));
    }
});
test('transport.consume() succeeds', async () => {
    ...
    expect(dump1.mapProducerIdConsumerIds).toEqual(expect.arrayContaining([
        { key: ctx.audioProducer.id, values: [audioConsumer.id] },
    ]));
    expect(dump1.mapConsumerIdProducerId).toEqual(expect.arrayContaining([
    ...
    expect(dump2.mapProducerIdConsumerIds).toEqual(expect.arrayContaining([
        {
    ...
    expect(dump2.mapConsumerIdProducerId).toEqual(expect.arrayContaining([
        { key: audioConsumer.id, value: ctx.audioProducer.id },
...
test('consumer.setPreferredLayers() with wrong arguments rejects with TypeError', async () => {
    ...
    await expect(
    // @ts-ignore
    videoConsumer.setPreferredLayers({ foo: '123' })).rejects.toThrow(TypeError);
    // @ts-ignore
    await expect(videoConsumer.setPreferredLayers('foo')).rejects.toThrow(TypeError);
...
test('consumer.enableTraceEvent() with wrong arguments rejects with TypeError', async () => {
    ...
    // @ts-ignore
    await expect(audioConsumer.enableTraceEvent('rtp')).rejects.toThrow(TypeError);
    await expect(
    // @ts-ignore
    audioConsumer.enableTraceEvent(['fir', 123.123])).rejects.toThrow(TypeError);
}, 2000);
...
test('consumer.close() succeeds', async () => {
    ...
    expect(routerDump.mapProducerIdConsumerIds).toEqual(expect.arrayContaining([
